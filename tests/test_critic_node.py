...
def test_critic_reports(tmp_path: Path) -> None:
    ...
    from json import loads  # local import to keep top block tidy

    from notebook_refactor_agent.agent.nodes.refactor import refactor_node as _refactor

    _refactor({"input_nb": str(p), "plan": plan, "output_dir": str(out_dir), "mode": "both"})
    ...
    data = loads((reports / "report.json").read_text())
